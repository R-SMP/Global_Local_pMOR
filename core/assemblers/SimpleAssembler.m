classdef SimpleAssembler < Assembler %#ok<*SPRIX>
    ...
    methods (Static)
        ...
        function [stiffnessMatrix, reducedStiffnessMatrix] = assembleGlobalStiffnessMatrix(femModel, modelPartName)
            ...
            conditions = femModel.getAllConditions();
            for c = conditions
                contrib = c.computeKContribution();
                ids = c.getDofList().getId();
                stiffnessMatrix(ids,ids) = stiffnessMatrix(ids,ids) + contrib;
            end
            ...
            elseif nargin == 2
                ...
                [free, ~] = femModel.getDofConstraints();
                stiffnessMatrix(~mp_dof_ids,~mp_dof_ids) = 0;
                reducedStiffnessMatrix = stiffnessMatrix(free.getId(),free.getId());
        ...
        function [forceVector, reducedForceVector] = applyExternalForces(femModel)
            ...
            conditions = femModel.getAllConditions();
            for c = conditions
                contrib = c.computeRHSContribution();
                ids = c.getDofList().getId();
                forceVector(ids) = forceVector(ids) + contrib;
            end
        ...
        function [massMatrix, reducedMassMatrix] = assembleGlobalMassMatrix(femModel, modelPartName)
            ...
            conditions = femModel.getAllConditions();
            for c = conditions
                contrib = c.computeMContribution();
                ids = c.getDofList().getId();
                massMatrix(ids,ids) = massMatrix(ids,ids) + contrib;
            end
            ...
            elseif nargin == 2
                ...
                [free, ~] = femModel.getDofConstraints();
                massMatrix(~mp_ids,~mp_ids) = 0;
                reducedMassMatrix = massMatrix(free.getId(),free.getId());
            end
            
        end
        
        function [dampingMatrix, reducedDampingMatrix] = assembleGlobalDampingMatrix(femModel, modelPartName)
            if nargin == 1
                elements = femModel.getAllElements;
            elseif nargin == 2
                elements = femModel.getModelPart(modelPartName).getElements;
            ...
            conditions = femModel.getAllConditions();
            for c = conditions
                contrib = c.computeCContribution();
                ids = c.getDofList().getId();
                dampingMatrix(ids,ids) = dampingMatrix(ids,ids) + contrib;
            end
            ...
            if nargin == 1
                ...
                if ~ isempty(fixedDofs)
                    fixedDofIds = fixedDofs.getId();
                    reducedDampingMatrix = applyMatrixBoundaryConditions(dampingMatrix, fixedDofIds);
                else
                    reducedDampingMatrix = dampingMatrix;
                end
            
            elseif nargin == 2
                mp_ids = femModel.getModelPart(modelPartName).getDofArray().getId();
                [free, ~] = femModel.getDofConstraints();
                dampingMatrix(~mp_ids,~mp_ids) = 0;
                reducedDampingMatrix = dampingMatrix(free.getId(),free.getId());
            end
