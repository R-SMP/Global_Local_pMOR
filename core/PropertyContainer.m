classdef PropertyContainer < handle
    ...
    methods
        ...
        function setValue(propertyContainer, name, value)
            %SETVALUE sets the value NAME in the property container. 
            %An already existing value is overwritten, otherwise a new
            %value is created.
            if propertyContainer.hasValue(name)
        ...
        function appendValue(obj, name, value)
            try
                vals = obj.propertyMap(name);
            catch
                error('The value \"%s\" is not available in this container', name)
            end
            
            vals(end+1) = value;
            obj.setValue(name, vals);
        end
        ...
        function value = getValue(obj, name, step)
            ...
            if obj.hasValue(name)
                ...
                if strcmp(checkPropertyName(name),'variable1d')
                    value = 0;
                elseif strcmp(checkPropertyName(name),'variable3d')
                    value = [0 0 0];
                end
            end
        end
