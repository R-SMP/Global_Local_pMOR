classdef Dof < handle
    ...
    methods
        ...
        function res = isFixed(obj)
            res = false(1,length(obj));
            for ii = 1:length(obj)
                res(ii) = obj(ii).fixed;
            end
        end
        
        function res = isPrescribed(obj)
            res = false(1,length(obj));
            for ii = 1:length(obj)
                res(ii) = obj(ii).prescribed;
            end
        end
        ...
        function prescribe(dofs, values)
            for ii = 1:length(dofs)
                dofs(ii).prescribed = true;
                dofs(ii).setValue(values(ii))
            end
        end

        function unprescribe(dofs)
            for ii = 1:length(dofs)
                dofs(ii).prescribed = false;
            end
        end
