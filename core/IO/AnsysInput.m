classdef AnsysInput < ModelIO
    ...
    methods
        ...
        function model = readModel(obj)
            ...
            [data.elementsOfModel,data.nodeElementList,data.nodeConnectivity,data.elementTypes] = AnsysInput.readElements();
            % Read surface loads
            ...
            e.setElementTable(data.elementsOfModel);
            ...
            % TODO: this should be a condition now
            ...
            e.setNodeConnectivity(data.nodeConnectivity,data.elementTypes);
    ...
    methods (Static)
        ...
        function runAnsys(ansysExecutable,folder,file,extension)
            ...
            % Run ANSYS
            command = ['"' ansysExecutable '" -b  -i DataAnsys/modelFile.txt -o DataAnsys/result.out'];
            % if needed, increase KMP_STACKSIZE here:
            [status,~] = system(['SET KMP_STACKSIZE=16m & ' command]);
            
            if status == -1073741571
                msg = ['AnsysInput: OpenMP stack too small. Increase ', ...
                    'stack size for the ANSYS call in AnsysInput.m.'];
                %see also https://de.mathworks.com/matlabcentral/answers/160210-how-can-i-run-ansys-from-matlab
                %and http://www.bgu.ac.il/intel_fortran_docs/compiler_f/main_for/mergedProjects/optaps_for/common/optaps_par_var.htm
                    e = MException('MATLAB:bm_mfem:ANSYSError',msg);
                throw(e);
            elseif status > 0
                msg = ['AnsysInput: ANSYS returned exit code ', ...
                    num2str(status), '. Check your input.'];
                e = MException('MATLAB:bm_mfem:ANSYSError',msg);
                throw(e);
            end
        ...
        function [dofs, nnodes, etype] = getAnsysElementInfo(elementName, keyopts)
...
%                     end
        ...
        function [elementList,nodesArrays,nodeConnectivity,elementType] = readElements()
            ...
            %              elementType  - array with element ids in (:,1)
            %                             and element type ids in (:,2)
            ...
            while ii < length(eledat)
                ...
                elementType(end+1,:) = [eledat(ii) etype]; %#ok<AGROW>
