...
xlim([0 10000])
ylim([1e-8 1e-1])
title('Frequency response using Global MOR')
ylabel('Displacement at load point')
xlabel('Frequency (rad/s)')
...
xlim([0 10000])
ylim([1e-8 1e-1])
legend('Full system', 'Reduced System (GLOBAL MOR)')
title('Comparison of full and reduced models')
ylabel('Displacement at load point')
subplot(2,1,2)
...
xlim([0 10000])
ylim([1e-8 1e-1])
legend('Full system', 'Reduced System (LOCAL MOR)')
title('Comparison of full and reduced models')
ylabel('Displacement at load point')
subplot(2,1,2)
...
xlim([0 10000])
ylim([1e-12 100])
legend('Reduced system error (LOCAL PMORT)',...
    'Location','northeast')
ylabel('Relative approximation error')
xlabel('Frequency (rad/s)')

% %% Local pMOR with different lengths
% 
% % Choose and initaliaze parameter space
% %Parameter is length, switch to height later
% 
...
% ff = zeros(r,1);
% Cf = zeros(1,r);
% p = [0.25 0.5 0.75 1 1.25]; % parameter for which interpolation is required, here L = 1m
% numParam = length(P);
% 
% %Use cubic splines: interpolate between the reduced and transformed
% %matrices to get the final system matrices corresponding to parameter 'p'
...
%         % My, Dy, Ky, fy, Cy are vectors with matrix elements corresponding to parameters in P 
%         fy = fgp(i,:);
%         ff(i) = spline(P,fy,p(k));
%         Cy = Cgp(:,i);
%         Cf(i) = spline(P',Cy,p(k));
%     
%         My = zeros(1,numParam);
%         Ky = zeros(1,numParam);
%         Dy = zeros(1,numParam);
...
%             end
%     
%             Mf(i,j) = spline(P,My,p(k));
%             Kf(i,j) = spline(P,Ky,p(k));
%             Df(i,j) = spline(P,Dy,p(k));
%         
%         end
%     end
%     
...
%    
%     for i = 1:length(s)
%        result_local(k,i) = Cf * ((s(i)^2*Mf + s(i)*Df + Kf) \ ff);
%     end
% 
% end
% 
...
% % 
% % %Error calculations
% % error_global = (result_full - result_local)./result_full;
% % error_norm_global = norm(result_full - result_local)/norm(result_full);
% % 
% % %Plot overlaid results and error plot for GLOBAL BASIS
% % fig = figure();
% % set(fig,'defaulttextinterpreter','latex')
...
% for i = 1:length(p)
% 
% semilogy(abs(s),abs(result_local(i,:)))
% hold on
% 
% end
% 
