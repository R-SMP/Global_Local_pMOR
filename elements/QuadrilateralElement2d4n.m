classdef QuadrilateralElement2d4n < QuadrilateralElement
    ...
    %   The element can be used in plane stress or plane strain
    %   configuration. Plane stress is the default behavior, plane strain
    %   can be enabled by setting the property USE_PLANE_STRAIN to true.
    ...
    methods
        ...
        function stiffnessMatrix = computeLocalStiffnessMatrix(obj)
            ...
            if obj.getProperties().hasValue('USE_PLANE_STRAIN') ...
                && obj.getPropertyValue('USE_PLANE_STRAIN')
            
                Emat = EModul/((1+prxy)*(1-2*prxy))*[1-prxy prxy 0; prxy 1-prxy 0; 0 0 (1 - 2*prxy)/2];
                Emat = sparse(Emat);
            else
