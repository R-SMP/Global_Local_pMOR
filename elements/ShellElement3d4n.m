classdef ShellElement3d4n < QuadrilateralElement 
    ...
    properties (Access = private)
        localCoordinates    %local coordinates transformed to x-y plane
    ...
    methods
        ...
        function initialize(obj)
            [tMat, m] = obj.getTransformationMatrix();
            c1 = tMat * (obj.nodeArray(1).getCoords - m)';
            c2 = tMat * (obj.nodeArray(2).getCoords - m)';
            c3 = tMat * (obj.nodeArray(3).getCoords - m)';
            c4 = tMat * (obj.nodeArray(4).getCoords - m)';
            ele_coords = [c1 c2 c3 c4]';
            obj.localCoordinates = ele_coords(:,1:2);
            obj.transformationMatrix = tMat;
            
            obj.checkConvexity();
        end
        
        function c = checkConvexity(obj)
            %SHELLELEMENT3D4N.CHECKCONVEXITY returns true, if the element
            %is convex
            diag1X = [obj.localCoordinates(1,1) obj.localCoordinates(3,1)];
            diag1Y = [obj.localCoordinates(1,2) obj.localCoordinates(3,2)];
            diag2X = [obj.localCoordinates(2,1) obj.localCoordinates(4,1)];
            diag2Y = [obj.localCoordinates(2,2) obj.localCoordinates(4,2)];
        ...
        function [N, N_mat, J] = computeShapeFunction(obj, xi, eta)
            ...
            % Jacobian
            J = N_Diff_Par * obj.localCoordinates;
        end
        
        function [B_mem, B_b] = computeBMatrix(obj, xi, eta, J)
            
            % Local coordinates of the nodes forming one element
            ele_coords = obj.localCoordinates;
        ...
        function stiffnessMatrix = computeLocalStiffnessMatrix(obj)
            ...
            stiffnessMatrix = T' * stiffnessMatrix * T;
            
        end
        
        
        function massMatrix = computeLocalMassMatrix(obj)
            density = obj.tryGetPropertyValue('DENSITY');
            thickness = obj.getPropertyValue('THICKNESS');
            if obj.getProperties().hasValue('USE_CONSISTENT_MASS_MATRIX')
