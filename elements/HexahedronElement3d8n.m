classdef HexahedronElement3d8n < Element  %Class Hexahedron to be implemented
    ...
    methods
        ...
        function hexahedron3d8n = HexahedronElement3d8n(id, nodeArray)
            ...
            requiredPropertyNames = cellstr(["YOUNGS_MODULUS", "POISSON_RATIO","NUMBER_GAUSS_POINT"]);
        ...
        function massMatrix = computeLocalMassMatrix(obj)
            ...
            for i=1:p
                ...
                for j=1:p
                    ...
                    for k=1:p
                        mue=g(k);
                        [N, ~, Jdet] = computeShapeFunction(obj, zeta, eta, mue);
                        massMatrix=massMatrix+(w(i)*w(j)*w(k)*rho*transpose(N)*N*Jdet);
                    end
                end
            end
