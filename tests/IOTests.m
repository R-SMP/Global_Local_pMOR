classdef IOTests <  matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function inputFileNotFoundTest(testCase)
        %INPUTFILENOTFOUNDTEST tests if an error is thrown if no input file
        %   can be located
            testCase.verifyError(@() MdpaInput('tests/input_data/not_existing.mdpa'), ...
                'MATLAB:bm_mfem:fileNotFound');
        end
        ...
        %ANSYSINPUTTEST tests input from an ANSYS model in a modal analysis
            ...
            %eigenvalue analysis
            io = AnsysInput('tests/input_data/ansys_model.inp', getPaths('ansys'));
            model = io.readModel;
